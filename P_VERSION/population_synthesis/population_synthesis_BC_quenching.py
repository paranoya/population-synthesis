...
# -*- coding: utf-8 -*-
"""
Created on Fri Nov 29 17:50:42 2019
...
#-----------------------------------------------------------------------------
SFH = 'tau_delayedEXPSFR_BC_quenched'
...
M_inf =1    
M_gas = M_inf                          # Msun
tau = np.logspace(-1, 1.7, 50)     # Gyr
tau = tau[::-1]
t_quenching=13.647
quenching_epoch = str(t_quenching)
...
# Output's path
SFHfolder = SFH+'/'
SEDfolder= SFHfolder + 'quenching_epoch_'+quenching_epoch+'Gyr' # For further classification
...
# Internal parameters
galaxy = SPiCE_galaxy.Point_source(today=13.648, R0=100)
...
for IMF_i in IMF:
  ...
  for Z_i in SSP.metallicities:
    ...
    for i, tau_i in enumerate(tau): 
      ...
      CEM = SPiCE_CEM.Exponential_quenched_SFR(M_gas=M_gas, Z=Z_i, M_inf=M_inf,
                                               tau=tau_i, t_quench=t_quenching)
      
      filename = IMF_i + '_Z_'+format(Z_i,'.4f') + '_tau_'+format(tau_i, '08.4f')
      print(filename)
