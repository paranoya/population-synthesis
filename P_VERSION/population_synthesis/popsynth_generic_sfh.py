...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb 10 16:16:22 2020
...
tau =  np.logspace(-.7, 1.7, 50)    # EXP & DEL-EXP


class SFH_model(object):
    def __init__(self, tau):
        self.tau = tau*units.Gyr        
        self.M_inf = units.Msun        
        
    def SFH(self, t):        
#          return t*np.exp(-(t-self.mu)**2/self.sigma**2)*units.Msun/units.Gyr**2
        return self.M_inf/self.tau *(t/self.tau)**np.exp(-t/self.tau)
...
# =============================================================================
SFHfolder = SFH+'/'
SEDfolder= SFHfolder + 'epoch'+str(t_today)+'Gyr'         # For further classification
...
for IMF_i in IMF:
  ...
  for i, param_i in enumerate(tau): 
      
      
      sfr = SFH_model(param_i)
      
      CEM = SPiCE_CEM.Generic_model(sfr=sfr.SFH, R=returned_gas_frac, z_sn=z_sn,  w=w)
          
      filename = IMF_i + '_beta_'+format(param_i, '08.4f')
...
#          ax.set_xscale('log')
          ax.set_yscale('log')
          ax.set_ylim( [1e-6, 2*max(Mass)] )
          ax.plot( t, Mass, 'k-*', figure=fig )
          ax.plot( t, Mass_metals, 'k--', figure=fig )
          ax.plot( t, Mass_gas, 'k-', figure=fig )
          ax1=ax.twinx()
          ax1.plot( t, met_Z/0.02, 'b-*', figure=fig )
          ax1.plot( t, met_Z_gas/0.02, 'b-', figure=fig )
      ...
      nu_Lnu_erg_s_stel = SSP.wavelength*SED / (units.erg/units.second)
      nu_Lnu_erg_s_neb = SSP.wavelength*SED / (units.erg/units.second)
      nu_Lnu_erg_s_Tot = SSP.wavelength*SED / (units.erg/units.second)
...
data = {'stellar_age':all_stellar_age,
        'Z_stars':all_z_star,
        'Z_gas':all_z_gas,
...
df.to_csv(globparams_folder+'params_t_{:4.2}.csv'.format(galaxy.today/units.Gyr))
