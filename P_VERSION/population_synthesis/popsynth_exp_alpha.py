...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb  6 11:10:06 2020
...
w = 2
phi_0 = 1    
alpha =  np.linspace(-1, 3, 100)    # EXP & DEL-EXP
...
for IMF_i in IMF:
  ...
  for i, alpha_i in enumerate(alpha): 
      ...
      CEM = SPiCE_CEM.Exponential_alpha(phi_0=phi_0, alpha=alpha_i,
                                        R=returned_gas_frac, z_sn=z_sn,  w=w)
          
      filename = IMF_i + '_alpha_'+format(alpha_i, '08.4f')
...
#          ax.set_xscale('log')
          ax.set_yscale('log')
          ax.set_ylim( [1e-6, 3] )
          ax.plot( t, Mass/max(Mass), 'k-*', figure=fig )
          ax.plot( t, Mass_metals/max(Mass_metals), 'k--', figure=fig )
          ax.plot( t, Mass_gas/max(Mass_gas), 'k-', figure=fig )
          ax1=ax.twinx()
