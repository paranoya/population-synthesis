    ...
    for i, Z in enumerate(self.metallicities):
	       for j, age in enumerate(self.log_ages_yr):
                 ...
                 self.SED_data[IMF][i][j] = np.loadtxt(file, dtype=np.float, skiprows=0, usecols=(1,2,3), unpack=True) \
		            * (3.828e33*units.erg/units.second/units.Angstrom) / units.Msun
...
class BC03_Padova94:
    ...
    metallicities = np.array([ 0.0001, 0.0004, 0.004, 0.008, 0.02, 0.05 ])
    ages = np.loadtxt(os.path.join(path, 'ages.dat'))
    log_ages_yr = np.log10(ages)
    
    SED_data = {}       

    def __init__(self, mode, IMF):         
        self.wavelength = np.loadtxt(os.path.join(BC03_Padova94.path, 
                                     'wavelength_'+mode+'.dat'))*units.Angstrom
        if IMF not in self.SED_data:
            print("> Initialising Bruzual&Charlote2003 (P94) models (IMF='"+IMF+"')")
            self.SED_data[IMF] = np.empty( shape=(self.metallicities.size,
                   self.log_ages_yr.size, self.wavelength.size), dtype=np.ndarray )
        for i, Z in enumerate(self.metallicities):	       
                 file = os.path.join( BC03_Padova94.path, IMF, 'SED',
	               'bc03_{0}_Z_{1:.4f}_{2}.txt'.format(mode, Z, IMF) )
                 self.SED_data[IMF][i][:,:] = np.loadtxt(file, dtype=np.float, ) \
		            * (3.828e33*units.erg/units.second/units.Angstrom) / units.Msun
