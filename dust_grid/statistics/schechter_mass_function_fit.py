...
#def schechter_dn_dlog10M(M, Mprime, alpha, phi=1):
#    return phi * np.exp(-(M/Mprime))/Mprime * (M/Mprime)**alpha * M
...
#    ax1 = plt.subplot(1, 1, 1)
    ...
    hh_u_r, mass_bins = np.histogram(mass_met_bins_u_r[i], range=[1e8, 1e12],
                                        weights=SDSS_V_max, bins=30,
                                        density=True
                                        )
    mass_bins = (mass_bins[1:]+mass_bins[:-1])/2
    total_galaxy_density = np.sum(SDSS_V_max[~np.isnan(mass_met_bins_u_r[i])])
    plt.plot(mass_bins, hh_u_r*total_galaxy_density,
             color=color_met[i],
             label=r'{:.2} $Z_\odot$'.format(models.metallicities[Z]/0.02))

    hh_g_r, mass_bins = np.histogram(mass_met_bins_g_r[i], range=[1e8, 1e12],
                                        weights=SDSS_V_max, bins=30,
                                        density=True
                                        )
    mass_bins = (mass_bins[1:]+mass_bins[:-1])/2
    total_galaxy_density = np.sum(SDSS_V_max[~np.isnan(mass_met_bins_g_r[i])])
    plt.plot(mass_bins, hh_g_r*total_galaxy_density,
             '--', color=color_met[i],
             label=r'{:.2} $Z_\odot$'.format(models.metallicities[Z]/0.02))

    plt.plot(mass_vector, schechter_mass_function(mass_vector,
             10**(11), alpha=-1.55, phi=.01), '--k')
#    plt.plot(np.log10(mass_vector), GAMA_dn_dlog10M(mass_vector/.7)*5/.7**3, '-k')
...
#    plt.ylim(3e-5, 3)
    plt.yscale('log')
    plt.xscale('log')
    plt.ylabel(r'$\frac{dN}{dM_*}$', fontsize=17)
    plt.xlabel(r'$\log(M/M_\odot)$', fontsize=15)
    plt.legend()
    plt.tick_params(which='both', length=4, direction='in',
                    right=True, top=True, labelsize=13, color='black')
