...
def running_mass_indices(data):
    cumulative = np.cumsum(data, axis=1)
    norm = cumulative[:, -1]
    cumulative = cumulative / norm[:, np.newaxis]
    return(np.argmin((cumulative-0.1)**2, axis=1),
           np.argmin((cumulative-0.5)**2, axis=1),
           np.argmin((cumulative-0.9)**2, axis=1))
...
bins = [np.linspace(8, 12, 30), np.logspace(-0.7, 1.71, 40)]
...
for i, Z in enumerate(GOOD_MET):
    ...
    hist_tau, logmass_edges, tau_edges = np.histogram2d(logmass_met_u_r[i],
                                                        tau_u_r_met_bins[i], 
                                                        weights = SDSS_V_max,
                                        bins=bins, range=[[8,12],[0.2, 51]],
                                        normed=True)
    
    lev  = [np.max(hist_tau)*0.01, np.max(hist_tau)*0.05, np.max(hist_tau)*0.1, 
        np.max(hist_tau)*0.3, np.max(hist_tau)*0.4, np.max(hist_tau)*0.5,
        np.max(hist_tau)*0.6, np.max(hist_tau)*0.7,
        np.max(hist_tau)*1]
    
    plt.contourf(logmass_edges[0:-1], tau_edges[0:-1], hist_tau.T,
                 cmap='Greys_r', levels=10)
    
    area = np.diff(logmass_edges)[:, np.newaxis] * np.diff(tau_edges)[np.newaxis, :]
    index_mass = running_mass_indices(hist_tau*area)
    for index in index_mass:
        plt.plot(logmass_edges[:-1], tau_edges[index], 'y-')
...
#    plt.plot(np.sort(logmass_met_u_r[i])[:90277], tau_p90_u_r[i][:90277], '-b')
    ...
    u_r_10 = np.interp(tau_p10_u_r[i], tau[::-1], u_r_model[i][::-1])
    u_r_90 = np.interp(tau_p90_u_r[i], tau[::-1], u_r_model[i][::-1])
    
    
    
    plt.scatter(np.sort(logmass_met_u_r[i])[:90277:100], tau_p50_u_r[i][:90277:100], 
                c=u_r_median[:90277:100],
                cmap='rainbow', norm=mcolors.Normalize(vmin=1.0, vmax=3.), 
                s=0.5)
    plt.scatter(np.sort(logmass_met_u_r[i])[:90277:100], tau_p90_u_r[i][:90277:100], 
                c=u_r_90[:90277:100],
                cmap='rainbow', norm=mcolors.Normalize(vmin=1.0, vmax=3.), 
                s=0.5)
    plt.scatter(np.sort(logmass_met_u_r[i])[:90277:100], tau_p10_u_r[i][:90277:100], 
                c=u_r_10[:90277:100],
                cmap='rainbow', norm=mcolors.Normalize(vmin=1.0, vmax=3.), 
                s=0.5)
    ...
    if i==0:
        plt.ylabel(r'$\tau$ [Gyr]', fontsize=14)
    else:        
...
#    plt.plot(np.sort(logmass_met_g_r[i])[:90277], tau_p90_g_r[i][:90277], '-b')
    ...
    g_r_10 = np.interp(tau_p10_g_r[i], tau[::-1], g_r_model[i][::-1])
    g_r_90 = np.interp(tau_p90_g_r[i], tau[::-1], g_r_model[i][::-1])
    ...
    plt.scatter(np.sort(logmass_met_g_r[i])[:90277], tau_p50_g_r[i][:90277], 
                ...
                s=0.5)
    plt.scatter(np.sort(logmass_met_g_r[i])[:90277], tau_p90_g_r[i][:90277], 
                c=g_r_90[:90277],
                cmap='rainbow', norm=mcolors.Normalize(vmin=.2, vmax=1.), 
                s=0.5)
    plt.scatter(np.sort(logmass_met_g_r[i])[:90277], tau_p10_g_r[i][:90277], 
                c=g_r_10[:90277],
                cmap='rainbow', norm=mcolors.Normalize(vmin=.2, vmax=1.), 
                s=0.5)
...
plt.savefig('tau_mass_distrib_met_bins_lin.png')
...
for i, Z in enumerate(GOOD_MET):
    ax1 = plt.subplot(2,4, i+1)
    hist_tau, logmass_edges, tau_edges = np.histogram2d(logmass_met_u_r[i],
                                                        np.log10(tau_u_r_met_bins[i]), 
                                                        weights = SDSS_V_max,
                                        bins=30, range=[[8,12],[-0.7, 1.8]] )
...
#    plt.plot(np.sort(logmass_met_u_r[i])[:90277], tau_p90_u_r[i][:90277], '-b')
    ...
    plt.scatter(np.sort(logmass_met_u_r[i])[:90277], np.log10(tau_p50_u_r[i][:90277]), 
                c=u_r_median[:90277],
                cmap='rainbow', norm=mcolors.Normalize(vmin=1.0, vmax=3.), 
                s=0.5)
    plt.scatter(np.sort(logmass_met_u_r[i])[:90277], np.log10(tau_p90_u_r[i][:90277]), 
                c=u_r_90[:90277],
                cmap='rainbow', norm=mcolors.Normalize(vmin=1.0, vmax=3.), 
                s=0.5)
    plt.scatter(np.sort(logmass_met_u_r[i])[:90277], np.log10(tau_p10_u_r[i][:90277]), 
                c=u_r_10[:90277],
                cmap='rainbow', norm=mcolors.Normalize(vmin=1.0, vmax=3.), 
                s=0.5)
    ...
    plt.annotate(r'{:.2} $Z_\odot$'.format(models.metallicities[Z]/0.02), 
                 xy=(10.5   , 1.3), color='white', weight='bold')
    ...
    plt.tick_params(which='both', length=4, direction = 'in',  right = True, top=True, labelsize=13, color='white')    
    if i==0:
        plt.ylabel(r'$\log(\tau/Gyr)$', fontsize=14)
    else:        
        plt.setp(ax1.get_yticklabels(), visible=False)
    if i==3:
        cbar = plt.colorbar()
    ...
    ax2 = plt.subplot(2,4, i+5)
    hist_tau, logmass_edges, tau_edges = np.histogram2d(logmass_met_g_r[i],
                                                np.log10(tau_g_r_met_bins[i]), 
                                                        weights = SDSS_V_max,
                                        bins=40, range=[[8,12],[-0.7, 1.8]] )
...
#    plt.plot(np.sort(logmass_met_g_r[i])[:90277], tau_p90_g_r[i][:90277], '-b')
    ...
    plt.scatter(np.sort(logmass_met_g_r[i])[:90277], np.log10(tau_p50_g_r[i][:90277]), 
                c=g_r_median[:90277],
                cmap='rainbow', norm=mcolors.Normalize(vmin=.2, vmax=1.), 
                s=0.5)
    plt.scatter(np.sort(logmass_met_g_r[i])[:90277], np.log10(tau_p90_g_r[i][:90277]), 
                c=g_r_90[:90277],
                cmap='rainbow', norm=mcolors.Normalize(vmin=.2, vmax=1.), 
                s=0.5)
    plt.scatter(np.sort(logmass_met_g_r[i])[:90277], np.log10(tau_p10_g_r[i][:90277]), 
                c=g_r_10[:90277],
                cmap='rainbow', norm=mcolors.Normalize(vmin=.2, vmax=1.), 
                s=0.5)
...
plt.subplots_adjust(hspace=0.0, wspace=0.0)    
plt.savefig('tau_mass_distrib_met_bins_log.png')
...
#    plt.plot(np.sort(logmass_met_u_r[i])[:90277], ssfr_p90[:90277], '-b')
    ...
    plt.scatter(np.sort(logmass_met_u_r[i])[:90277], ssfr_p50[:90277], 
                ...
                cmap='rainbow', norm=mcolors.Normalize(vmin=1.0, vmax=3.), 
                s=0.5)
    plt.scatter(np.sort(logmass_met_u_r[i])[:90277], ssfr_p90[:90277], 
                c=u_r_90[:90277],
                cmap='rainbow', norm=mcolors.Normalize(vmin=1.0, vmax=3.), 
                s=0.5)
    plt.scatter(np.sort(logmass_met_u_r[i])[:90277], ssfr_p10[:90277], 
                c=u_r_10[:90277],
                cmap='rainbow', norm=mcolors.Normalize(vmin=1.0, vmax=3.), 
                s=0.5)
    ...
    plt.tick_params(which='both', length=4, direction = 'in',  right = True, top=True, labelsize=13, color='white')    
    if i==0:
        plt.ylabel(r'$\log(sSFR/[1/yr])$', fontsize=14)
    else:        
        plt.setp(ax1.get_yticklabels(), visible=False)
    if i==3:
        cbar = plt.colorbar()
...
#    plt.plot(np.sort(logmass_met_g_r[i])[:90277], ssfr_p90[:90277], '-b')
    ...
    plt.scatter(np.sort(logmass_met_g_r[i])[:90277], ssfr_p50[:90277], 
                ...
                cmap='rainbow', norm=mcolors.Normalize(vmin=.2, vmax=1.), 
                s=0.5)
    plt.scatter(np.sort(logmass_met_g_r[i])[:90277], ssfr_p90[:90277], 
                c=g_r_90[:90277],
                cmap='rainbow', norm=mcolors.Normalize(vmin=.2, vmax=1.), 
                s=0.5)
    plt.scatter(np.sort(logmass_met_g_r[i])[:90277], ssfr_p10[:90277], 
                c=g_r_10[:90277],
                cmap='rainbow', norm=mcolors.Normalize(vmin=.2, vmax=1.), 
                s=0.5)
...
#        plt.setp(ax1.get_xticklabels(), visible=False)
        ...
        if i_mass==0:
            plt.ylabel(r'$\frac{dp}{d\tau}$', fontsize=14)        
        else:
            plt.setp(ax1.get_yticklabels(), visible=False)
...
#        plt.yscale('log')
        ...
        if i_mass==0:
            plt.ylabel(r'$\frac{dp}{dsSFR}$', fontsize=14)
        else:
            plt.setp(ax2.get_yticklabels(), visible=False)
