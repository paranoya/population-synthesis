...
output_path = '../../GAMA_data/products/'
...
if dust_grid ==False:                
    all_lick_indices = []
    all_lick_indices_err = []
    all_photometry = []
    for i_element in range(len(SEDpaths)):
#    for i_element in range(5):
#------------------------------------------------------------------------------
# Modify this code block for obtaining the different input spectra parameters.        
        print('\n {:}'.format(i_element/len(SEDpaths)*100)+' % complete -->' \
              + SEDpaths[i_element])
...
#        )
        ...
        all_lick_indices.append(lick_data)
        all_lick_indices_err.append(lick_err)
        all_photometry.append(photometric_data)
        


# Convertion of list into arrays with dim 0 being the lick/photo list and 
# dim 1 being the number of galaxies
lick_idx.append(D4000)

all_lick_indices = np.array(all_lick_indices).T        
all_lick_indices_err = np.array(all_lick_indices_err).T

all_photometry = np.array(all_photometry).T
...
lick_data = {}
for i in range(all_lick_indices.shape[0]):
    lick_data[lick_idx[i]]=all_lick_indices[i,:] 
    lick_data[lick_idx[i]+'_err']=all_lick_indices_err[i,:] 
    
phot_data = {}
for i in range(all_photometry.shape[0]):
    phot_data[photo_bands[i]]=all_photometry[i,:]
    
dF_lick = pd.DataFrame(data=lick_data)
dF_lick.to_csv(output_path+'GAMA_lick_indices.csv')

dF_phot = pd.DataFrame(data=phot_data)
dF_phot.to_csv(output_path+'GAMA_photometry.csv')
